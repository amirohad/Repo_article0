...
class Plant:
    ...
    def __init__(self,df,basepath,i,exp):
        ...
        self.m_sup = float(df.at[i,'Straw_Weight(gr)']) # support mass in gr
...
class Event:
    ...
    def event_calc_variables(self,view):
        if view == 'side':
            ...
            self.alpha = [m.asin(d/((self.p.Lsup_cm-
                            self.L_track2suptip_cm))) for d in self.trk_dist] # old calculation
            self.alpha2 = [m.asin(d/(2*(self.p.Lsup_cm-
            ...
            self.F_bean2 = F_of_t(self.L_contact2suptip,
                      self.p.Lsup_cm, self.alpha2,self.p.m_sup)
            # calculate torque
...
def calc_F(d_contact,l_sup_cm,phi_t,m_sup):
    '''calculate force in milli?-Newton of bean on support via moment equilibrium equation.
        d_contact in cm, l_sup_cm in cm, phi_t in radians, m_sup in grams, g=980 cm/s^2'''
    gcgs=980
    dyne2mN = 1/100
    # F_mg = 1000*m_sup * l_sup_cm * m.tan(phi_t)/(2*(l_sup_cm-d_contact))
    F_mN = gcgs*m_sup * l_sup_cm * m.tan(phi_t)/(2*(l_sup_cm-d_contact))*dyne2mN
